...
const istaImage = [insta1, insta2, insta3, insta4, insta5, insta6];

export default function Follower() {
  const { t, dir } = useLanguage();

  return (
    <div className="follow" dir={dir}>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
        {istaImage.map((item, index) => (
          <div key={index} className="follow-img overflow-hidden">
            <Image src={item} alt={`insta-${index}`} />
          </div>
        ))}
      </div>

      <div className="follow-text flex items-center gap-4 bg-white p-3 rounded-2xl">
        <Image src={instaFollow} alt="insta-follow" />
        <h2 className="GolosText font-semibold" dir="ltr">
          {t.home.followUs}
        </h2>
      </div>
    </div>
